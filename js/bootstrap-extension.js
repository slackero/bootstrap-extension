/*
 * Bootstrap Extension v5.3.0 (https://bootstrap-extension.com)
...
docReady(() => {
    ...
    for (let i = 0; i < imageForInputs.length; i++) {
    ...
    for (let i = 0; i < hoverZoomImages.length; i++) {
            ...
            menuItem.addEventListener('click', (event) => {
                ...
                if (i > 0 && menuItem.getAttribute('href').substring(0, 1) === '#') {
                    ...
                    if (hp && getComputedStyle(hp).getPropertyValue('display') === 'flex') {
                        ...
                        var posY = document.querySelector(menuItem.getAttribute('href')).offsetTop - offset;
                    }
                    window.scrollTo({
                        top: posY,
                        behavior: 'smooth'
    ...
    for (let i = 0; i < wbLinks.length; i++) {
